<template>
  <div class="dictionary-content">
    ...
    <el-dialog v-if="dialogFormVisible" :visible.sync="dialogFormVisible">
      <dictionary-form :data="formData" @callback="onCallback" @cancel="dialogFormVisible=false" />
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    getRoot() {
      ...
      DictionaryModel.fetchRoot({ deleted: false }).then(response => {
    ...
    loadNode(node, resolve) {
      ...
      DictionaryModel.fetchByParentCode(node.data.code, { deleted: false }).then(response => {
    ...
    remove(node, data) {
      ...
      this.$confirm('是否删除节点?', '提示', {
        ...
        DictionaryModel.remove(data.id, true).then(response => {
          this.onCallback()
        })
      })
    },
    onCallback() {
      ...
      if (parentNode && parentNode.level > 0) {
        DictionaryModel.fetchByParentCode(parentNode.data.code, { deleted: false }).then(response => {
